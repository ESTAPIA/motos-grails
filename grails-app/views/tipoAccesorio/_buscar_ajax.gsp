...
<g:if test="${tipoAccesorios && tipoAccesorios.size() > 0}">
    <g:each in="${tipoAccesorios}" var="tipo">
        <tr data-id="${tipo.id}">
            <td>
                <span class="nombre-tipo">
                    ${tipo.tipoDeAccesorio ?: ''}
                </span>
            </td>
            <td>
...
<script>
    ...
    <g:else>
        $("#resultados").text("Mostrando ${totalSQL} tipo(s) de accesorio");
    </g:else>
</script>
